...
if __name__ == '__main__':
    ...
    results_dir = '/storage/coda1/p-hl94/0/schaudhary9/testflight_data/CUTGan/Results/CUTgan_' + str(run) + '_' + str(tsize)
    ...
    with tf.Session() as sess:
        ...
        for i in range(training_iters):
            if tsize > 1000:
                idx = random.sample(range(tsize), 300)
            ...
            for batch in range(len(curr_batch_X) // batch_size):
                ...
                # print("Iter-" + str(i) + ", disc_loss:" + "{:.6f}".format(c1) + + ", gen_loss:" + "{:.6f}".format(c2) + + ", cont_loss:" + "{:.6f}".format(c3))
            ...
            for k in range(20):
                ...
                curr_cont_loss, curr_gen_adv_loss, curr_disc_loss = sess.run([f_cont_loss, gen_adv_loss, disc_loss], feed_dict={x: test_X[idx_x[0:1], :, :, :], t: train_T[idx[0:1], :, :, :]})
            ...
            for k in range(20):
                ...
                idx_t = random.sample(range(len(train_T)), 1)
                curr_cont_loss, curr_gen_adv_loss, curr_disc_loss = sess.run([f_cont_loss, gen_adv_loss, disc_loss], feed_dict={x: train_X[idx_x[0:1], :, :, :], t: train_T[idx[0:1], :, :, :]})
