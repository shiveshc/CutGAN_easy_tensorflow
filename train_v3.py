...
if __name__ == '__main__':
    ...
    with tf.variable_scope('gen', reuse= tf.AUTO_REUSE):
        ...
        x_patch, x_hat_patch = get_patch_features(x, x_hat, generator, 256)
    ...
    for l in x_hat_patch.keys():
        ...
        for i in range(num_patches):
            z = z_x_hat[:, i, :]
            z_pos = z_x[:, i, :]
            z_neg_collec = [z_x[:, k, :] for k in range(num_patches) if k!= i]
            z_neg = z_neg_collec[0]
            for k in range(1, len(z_neg_collec)):
                z_neg = tf.concat([z_neg, z_neg_collec[k]], axis=0)

            cont_loss.append(get_ce_loss(z, z_pos, z_neg))

    f_cont_loss = tf.reduce_mean(tf.stack(cont_loss, axis= 0))
    ...
    results_dir = '/storage/coda1/p-hl94/0/schaudhary9/testflight_data/CUTGan/Results/CUTgan_train_v3_' + str(run) + '_' + str(tsize)
    ...
    with tf.Session() as sess:
        ...
        for i in range(training_iters):
            ...
            for k in range(5):
                idx_x = random.sample(range(len(test_X)), 10)
                idx_t = random.sample(range(len(train_T)), 10)
                curr_cont_loss, curr_gen_adv_loss, curr_disc_loss = sess.run([f_cont_loss, gen_adv_loss, disc_loss], feed_dict={x: test_X[idx_x[:10], :, :, :], t: train_T[idx[:10], :, :, :]})
            ...
            for k in range(5):
                idx_x = random.sample(range(len(train_X)), 10)
                idx_t = random.sample(range(len(train_T)), 10)
                curr_cont_loss, curr_gen_adv_loss, curr_disc_loss = sess.run([f_cont_loss, gen_adv_loss, disc_loss], feed_dict={x: train_X[idx_x[:10], :, :, :], t: train_T[idx[:10], :, :, :]})
        ...
        for i in range(10):
            ...
            cv2.imwrite(results_dir + '/X_' + str(temp_idx + 1) + '.png', temp_X[0, :, :, :])  # this is the middle zplane corresponding to gt zplane
            cv2.imwrite(results_dir + '/pred_' + str(temp_idx + 1) + '.png', temp_pred[0, :, :, :])
