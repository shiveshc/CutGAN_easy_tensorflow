...
def get_ce_loss(z_x_hat, z_x, num_patches):
    z_x_hat = tf.reshape(z_x_hat, [-1, z_x_hat.shape[2]])
    z_x = tf.reshape(z_x, [-1, z_x.shape[2]])
    logits = tf.matmul(z_x_hat, tf.transpose(z_x))
    labels = tf.eye(num_patches)
...
if __name__ == '__main__':
    ...
    for l in x_hat_patch.keys():
        ...
        cont_loss.append(get_ce_loss(z_x_hat, z_x, num_patches))
    ...
    # results_dir = 'D:/Shivesh/CUTGan/Results/CUTgan_train_v3_v2_' + str(run) + '_' + str(tsize)
    results_dir = '/storage/scratch1/0/schaudhary9/CUTGan/Results/CUTgan_train_v4_unet_stdgan_newtrain_' + str(run) + '_' + str(tsize)
