...
def get_ce_loss(z_x_hat, z_x, num_patches, batch_size):
    ...
    logits = tf.matmul(z_x_hat, tf.transpose(z_x, (0, 2, 1)))/0.07
...
if __name__ == '__main__':
    ...
    with tf.variable_scope('gen', reuse= tf.AUTO_REUSE):
        x_hat, _ = generator.conv_net(x, x.shape[3])
    ...
    mlp_opt = tf.train.AdamOptimizer(learning_rate=lr, beta1= 0.5).minimize(f_cont_loss, var_list= mlp_variables)

    init = tf.global_variables_initializer()
    saver = tf.train.Saver()

    # make folder where all results will be saved
    # results_dir = 'D:/Shivesh/CUTGan/Results/CUTgan_train_v4_v2_' + str(run) + '_' + str(tsize)
    results_dir = '/storage/scratch1/0/schaudhary9/CUTGan/Results/CUTgan_train_v5_unet_lsgan_stdtrain_batch_' + str(run) + '_' + str(tsize)
    ...
    with tf.Session() as sess:
        ...
        for i in range(training_iters):
            if tsize > 50:
                idx = random.sample(range(tsize), 50)
        ...
        for i in range(10):
            ...
            temp_pred = temp_pred[0, :, :, :] * 127.5 + 127.5
            temp_pred = temp_pred.astype(np.uint8)

            temp_X = temp_X[0, :, :, :] * 127.5 + 127.5
            temp_X = temp_X.astype(np.uint8)

            cv2.imwrite(results_dir + '/X_' + str(temp_idx + 1) + '.png', temp_X)
            cv2.imwrite(results_dir + '/pred_' + str(temp_idx + 1) + '.png', temp_pred)
